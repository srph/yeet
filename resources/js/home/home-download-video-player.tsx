 ...
 *
 * All chrome lives under a bottom scrim and is hidden until Vidstack says the
 * controls are visible, so a paused player reads exactly like the old
 * thumbnail. Seek bar sits on its own row above the buttons: the scrub target
 * is then the full width of the frame rather than whatever is left over
 * between the timecode and the volume toggle.
 *
 * Only mounted once status is "complete" and download_url is non-null —
 * HomeDownloadTracking keeps its shimmering thumbnail for every other state.
 */

/** Shared by both rows; 28px is the smallest that still clears a thumb. */
const ICON_BUTTON =
...
export function HomeDownloadVideoPlayer({
  ...
  return (
    <MediaPlayer
      ...
      playsInline
      // The poster is the same thumbnail the waiting state was showing, so
      // there is no visible swap when the job settles.
      poster={meta.source_thumbnail ?? undefined}
      onError={onSourceError}
      className="group relative block aspect-video w-full overflow-hidden rounded-2xl bg-neutral-950 shadow-[0_40px_90px_-30px_rgba(0,0,0,0.95),0_0_0_1px_rgba(191,219,254,0.34),0_0_90px_-26px_rgba(191,219,254,0.42)] focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-blue-200"
    >
      ...
      <Controls.Root
        className="absolute inset-0 z-20 flex flex-col justify-end opacity-0 transition-opacity duration-300 ease-[cubic-bezier(0.19,1,0.22,1)] data-[visible]:opacity-100"
        hideOnMouseLeave
      >
        ...
        <Controls.Group className="relative flex flex-col gap-[7px] px-[15px] pb-[13px]">
          <TimeSlider.Root className="group/seek relative flex h-[18px] w-full cursor-pointer touch-none items-center outline-none">
          ...
          <div className="flex items-center gap-[11px]">
            ...
            {/* <Time> renders a block-level div, so this wrapper has to be a
                            flex row — whitespace-nowrap cannot stop a block from taking
                            a line of its own. */}
            <span className="flex shrink-0 items-baseline gap-1 text-xs font-medium tracking-[-0.01em] text-white tabular-nums">
              <Time type="current" />
...
/**
 * The one control that is not in the dock. Was 74px with an offset shadow and
 * a white hairline ring; the ring did nothing on a near-white fill but muddy
 * the edge, and the offset made it read as pasted on. Now smaller, ringless,
 * and lit by a centred bloom so it separates from a bright frame without one.
 */
function CenterPlay() {
  ...
  return (
    <PlayButton
      className={`absolute inset-0 z-10 m-auto grid size-16 place-items-center rounded-full bg-blue-200 text-blue-950 shadow-[0_2px_30px_rgba(0,0,0,0.5)] transition duration-300 ease-[cubic-bezier(0.19,1,0.22,1)] hover:bg-blue-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-200 ${
        paused
...
function VolumeGlyph({ size }: { size: number }) {
  const muted = useMediaState("muted");
  const Glyph = muted ? VolumeXIcon : Volume2Icon;

  return <Glyph style={{ width: size, height: size }} />;
}
...
function FullscreenGlyph() {
  ...
  return <Glyph className="size-[15px]" />;
}
